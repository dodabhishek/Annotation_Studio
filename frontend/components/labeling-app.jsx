...
import { Tags, Layers, ArrowLeft, Sparkles, Loader2, AlertCircle, ImageIcon, X } from 'lucide-react';
...
export function LabelingApp({ initialFiles, selectedModel, onBack }) {
    ...
    return (<div className="h-screen flex flex-col bg-background">
      ...
      {/* Toolbar */}
      <Toolbar selectedTool={store.selectedTool} onSelectTool={store.setSelectedTool} zoom={store.zoom} onZoomIn={() => store.setZoom(Math.min(store.zoom * 1.2, 5))} onZoomOut={() => store.setZoom(Math.max(store.zoom / 1.2, 0.1))} onResetView={store.resetZoom} onDeleteSelected={() => {
            if (store.selectedAnnotationId) {
                store.deleteAnnotation(store.selectedAnnotationId);
            }
        }} onExport={handleExport} hasSelectedAnnotation={!!store.selectedAnnotationId} canGoNext={store.currentImageIndex < store.images.length - 1} canGoPrev={store.currentImageIndex > 0} onNext={store.nextImage} onPrev={store.prevImage} currentIndex={store.currentImageIndex} totalImages={store.images.length}/>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Canvas */}
        <AnnotationCanvas image={store.currentImage} annotations={store.currentImage?.annotations || []} labels={store.labels} selectedTool={store.selectedTool} selectedLabelId={store.selectedLabelId} selectedAnnotationId={store.selectedAnnotationId} zoom={store.zoom} pan={store.pan} onAddAnnotation={store.addAnnotation} onUpdateAnnotation={store.updateAnnotation} onSelectAnnotation={store.setSelectedAnnotationId} onZoomChange={store.setZoom} onPanChange={store.setPan}/>
        ...
        <aside className="w-72 border-l border-border bg-card flex-shrink-0 flex flex-col">
          ...
          <Tabs defaultValue="labels" className="flex flex-col h-full">
            ...
            <TabsContent value="labels" className="flex-1 m-0 overflow-hidden">
              <LabelManager labels={store.labels} selectedLabelId={store.selectedLabelId} onSelectLabel={store.setSelectedLabelId} onAddLabel={store.addLabel} onUpdateLabel={store.updateLabel} onDeleteLabel={store.deleteLabel}/>
            </TabsContent>
